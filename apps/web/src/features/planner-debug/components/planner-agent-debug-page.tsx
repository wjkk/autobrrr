...
export function PlannerAgentDebugPage({ initialSubAgentSlug, mode = 'debug', initialReplayRunId, initialAutoRun = false }: PlannerAgentDebugPageProps) {
  ...
  const handleSave = async (options?: { openDebugAfterSave?: boolean; autoRunAfterOpen?: boolean }) => {
    ...
    try {
      ...
      if (options?.openDebugAfterSave) {
        ...
        router.push(`/internal/planner-debug/${encodeURIComponent(selectedSubAgentEntry.subAgent.slug)}${search}`);
      }
    } catch (error) {
                  ...
                  {selectedSubAgentEntry ? (
                    <div className={styles.stack}>
                      ...
                      <div className={styles.fieldGroup}>
                        ...
                        <PlannerInputSchemaEditor
                          value={editorState.inputSchema}
                          onChange={(inputSchema) => setEditorState((current) => ({ ...current, inputSchema }))}
                        />
                        <details className={styles.jsonPreview}>
                          <summary className={styles.jsonPreviewSummary}>查看保存后的 JSON</summary>
                      ...
                      <div className={styles.fieldGroup}>
                        ...
                        <PlannerOutputSchemaEditor
                          value={editorState.outputSchema}
                          onChange={(outputSchema) => setEditorState((current) => ({ ...current, outputSchema }))}
                        />
                        <details className={styles.jsonPreview}>
                          <summary className={styles.jsonPreviewSummary}>查看保存后的 JSON</summary>
                      ...
                      <div className={styles.fieldGroup}>
                        ...
                        <PlannerToolPolicyEditor
                          value={editorState.toolPolicy}
                          onChange={(toolPolicy) => setEditorState((current) => ({ ...current, toolPolicy }))}
                        />
                        <details className={styles.jsonPreview}>
                          <summary className={styles.jsonPreviewSummary}>查看保存后的 JSON</summary>
                      ...
                      <div className={styles.fieldGroup}>
                        ...
                        <PlannerGenerationConfigEditor
                          value={editorState.generationConfig}
                          onChange={(generationConfig) => setEditorState((current) => ({ ...current, generationConfig }))}
                        />
                        <details className={styles.jsonPreview}>
                          <summary className={styles.jsonPreviewSummary}>查看保存后的 JSON</summary>
              ...
              <div className={styles.panel}>
                ...
                <div className={styles.threeCol}>
                  <div className={styles.fieldGroup}>
                    ...
                    <select
                      ...
                      value={debugForm.configSource}
                      onChange={(event) =>
                        setDebugForm((current) => ({
                          ...current,
                          configSource: event.target.value === 'published' ? 'published' : 'draft',
                        }))
                      }
                    >
                  ...
                  <div className={styles.fieldGroup}>
                    ...
                    <select
                      className={styles.select}
                      value={debugForm.targetStage}
                      onChange={(event) =>
                        setDebugForm((current) => ({
                          ...current,
                          targetStage: event.target.value === 'outline' ? 'outline' : 'refinement',
                          partialRerunScope: event.target.value === 'outline' ? 'none' : current.partialRerunScope,
                        }))
                      }
                    >
                  ...
                  <div className={styles.fieldGroup}>
                    ...
                    <select
                      className={styles.select}
                      value={debugForm.partialRerunScope}
                      disabled={debugForm.targetStage !== 'refinement'}
                      onChange={(event) =>
                        setDebugForm((current) => ({
                          ...current,
                          partialRerunScope: event.target.value as DebugFormState['partialRerunScope'],
                        }))
                      }
                    >
                ...
                <div className={styles.fieldGroup}>
                  ...
                  <textarea
                    ...
                    placeholder='[{"role":"user","text":"上一轮需求"},{"role":"assistant","text":"上一轮确认"}]'
                    value={debugForm.priorMessagesJson}
                    onChange={(event) => setDebugForm((current) => ({ ...current, priorMessagesJson: event.target.value }))}
                  />
                </div>
                ...
                <div className={styles.fieldGroup}>
                  ...
                  <textarea
                    className={styles.textarea}
                    placeholder='{"projectTitle":"...","storyArc":[...]}'
                    value={debugForm.currentOutlineDocJson}
                    onChange={(event) => setDebugForm((current) => ({ ...current, currentOutlineDocJson: event.target.value }))}
                  />
                </div>
                ...
                <div className={styles.fieldGroup}>
                  ...
                  <textarea
                    className={styles.textarea}
                    placeholder='{"subjects":[...],"scenes":[...],"acts":[...]}'
                    value={debugForm.currentStructuredDocJson}
                    onChange={(event) => setDebugForm((current) => ({ ...current, currentStructuredDocJson: event.target.value }))}
                  />
                </div>
                ...
                <div className={styles.fieldGroup}>
                  ...
                  <textarea
                    className={styles.textarea}
                    placeholder='{"id":"...","name":"目标主体"}'
                    value={debugForm.targetEntityJson}
                    onChange={(event) => setDebugForm((current) => ({ ...current, targetEntityJson: event.target.value }))}
                  />
                </div>
                ...
                <div className={styles.fieldGroup}>
                  ...
                  <textarea
                    className={styles.textarea}
                    placeholder='[{"id":"asset-1","sourceUrl":"https://...","sourceKind":"generated","createdAt":"2026-03-14T10:00:00.000Z"}]'
                    value={debugForm.plannerAssetsJson}
                    onChange={(event) => setDebugForm((current) => ({ ...current, plannerAssetsJson: event.target.value }))}
                  />
                </div>
                ...
                {debugResult ? (
                  <PlannerDebugResultView
                    ...
                    replayHref={`/internal/planner-debug/runs/${encodeURIComponent(debugResult.debugRunId)}`}
                    refillHref={
                      selectedSubAgentEntry
                        ? `/internal/planner-debug/${encodeURIComponent(selectedSubAgentEntry.subAgent.slug)}?replayRunId=${encodeURIComponent(debugResult.debugRunId)}`
                        : null
                    }
                  />
                ) : null}
              ...
              <div className={styles.panel}>
                ...
                <div className={styles.panelBody}>
                  <div className={styles.stack}>
                    ...
                    {compareResult ? <PlannerDebugCompareView compareResult={compareResult} /> : null}
                      ...
                      {recentRuns.map((run) => (
                        <button key={run.id} type="button" className={styles.historyRunItem} onClick={() => handleSelectRun(run.id)}>
                          <div className={styles.catalogTitle}>
                            ...
                            <span className={statusClass(run.executionMode)}>
                              {run.executionMode === 'live' ? '真实模型' : '回退生成'}
                            </span>
                          </div>
