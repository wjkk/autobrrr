...
但 Planner 可以输出这些阶段需要的输入草稿与控制参数。
...
4. `GET /api/projects/:projectId/planner/refinement/versions`
...
2. `GET /api/planner/sub-agent-profiles`
3. `POST /api/planner/sub-agent-profiles`
4. `PATCH /api/planner/sub-agent-profiles/:id`
5. `POST /api/planner/sub-agent-profiles/:id/publish`

### 12.3 调试接口

1. `POST /api/planner/debug/run`
2. `GET /api/planner/debug/runs/:runId`
3. `POST /api/planner/debug/runs/:runId/replay`
4. `POST /api/planner/debug/compare`
...
2. `/internal/planner-agents/:subAgentSlug`
3. `/internal/planner-debug/runs/:runId`
