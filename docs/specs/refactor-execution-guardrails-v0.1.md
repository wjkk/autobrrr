...
65. run worker 这类“把 queued/running run 真正送进 submit/poll/fail 分支”的执行桥接层也必须有 API 单测，至少锁住 providerData 合并、claimed run 的分流判定和 unsupported runType 硬失败；否则 provider 或 runType 扩容后最容易把任务 silently 路由到错误执行路径
...
45. planner debug 配置编辑器、分镜指针查找这类“前端薄纯逻辑”也必须有 web 单测；不要因为文件短就放过，这类逻辑一旦漂移，页面往往不会崩，但会在编辑器和对话框里悄悄写错状态
46. 对多个页面共用的 server wrapper fallback 语义，应优先收口成共享 helper 并集中测试，而不是让每个页面各写一份 try/catch；否则最容易出现某个入口返回 `[]`、另一个入口返回 `null`、第三个入口直接抛错的双轨行为
47. 复杂 hook 中凡是可提纯为“请求 helper / payload builder / catalog presenter”的逻辑，都应尽早下沉成独立模块并补单测；否则 hook 会继续膨胀，最终只能靠浏览器回归兜底
